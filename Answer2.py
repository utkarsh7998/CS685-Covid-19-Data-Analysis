...
df = anst.copy()
printed = []
def already_printed(x, y):
    for i in range(len(printed)):
        if printed[i]['District'] == y and printed[i]['Neighbors']==x:
            return False
    return True
for i in range(len(df)):
    district = df.loc[i,'District']
    neighbors = df.loc[i,'Neighbors']
    for j in neighbors.split(', '):
        d = {}
        d['District'] = district
        d['Neighbors'] = j
        if already_printed(district,j)==True and j!='':
            printed.append(d)
#             print(d) 
...
ans = pd.DataFrame(printed)
ans.sort_values('District',inplace=True)
ans.drop_duplicates(subset =['District','Neighbors'],keep = 'first', inplace = True)


# In[49]:


ans.to_csv('edge-graph.csv',index=False)
